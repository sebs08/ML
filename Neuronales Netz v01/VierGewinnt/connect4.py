...
class Game(object):
    ...
    def __init__(self, action_function):
        ...
        self.players[0] = Player('net', self.colors[0], action_function)
        #self.players[1] = Player('net', self.colors[1], action_function)

        self.players[1] = AIPlayer('AI', self.colors[1], AI_diff)
        ...
        self.turn = self.players[np.random.randint(0,2,1)[0]]
    ...
    def return_board_as_array(self):
        """
        board_array = []
        for i in range(6):
            board_array.append([])
        for i in range(6):
            for j in range(7):
                if self.board[i][j] == ' ':
                    board_array[5-i].append(0)
                elif self.board[i][j] == self.players[0].color:
                    board_array[5-i].append(1)
                elif self.board[i][j] == self.players[1].color:
                    board_array[5-i].append(2)
                elif self.board[i][j] == self.players[0].color.upper():
                    board_array[5-i].append(1)
                elif self.board[i][j] == self.players[1].color.upper():
                    board_array[5-i].append(2)
        """

        board_array = []
        for i in range(5,-1,-1): # rows
    ...
    def nextMove(self):
        ...
        if player.name == 'net':
            move = player.move(self.return_board_as_array())
        else:
...
class Player(object):
    ...
    def __init__(self, name, color, action_function):
        self.type = "NeuroNet"
        self.name = name
        self.color = color
        self.action_function = action_function # give function which evaluates the next move

    def move(self, state):
        #print("{0}'s turn.  {0} is {1}".format(self.name, self.color))
        column = self.action_function(state)
        return column
